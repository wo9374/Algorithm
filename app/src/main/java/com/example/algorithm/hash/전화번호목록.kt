...
class 전화번호목록 {
    ...
    fun solution(phone_book: Array<String>): Boolean {
        ...
        // 2. 모든 전화번호를 HashMap 에 넣기
        for (i in phone_book.indices) map[phone_book[i]] = i

        // 3. 모든 전화번호의 substring 이 HashMap 에 존재하는지 확인
        for (s in phone_book)
            for (j in s.indices)
                if (map.containsKey(s.substring(0, j))) //String 이라는 Key 가 현재 HashMap 에 존재하는지 확인하는 함수
                     return false
